...
| `REFUSAL_DOCUMENT_UNREADABLE` | A document named on the command line could not be read: the file is missing, unreadable, not JSON, or JSON nested deeper than the decoder can read. The detail names the path and what went wrong. |
