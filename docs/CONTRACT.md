...
| `REFUSAL_TEXT_HAS_CONTROL_CHARACTERS` | A text field carries a control character -- a NUL, a line break, a tab or another character whose Unicode category is Cc. Refused by name wherever the module reads text, so a driver never turns it into a configuration-class sentence. Letters in any script are text and are accepted. |
