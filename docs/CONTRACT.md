...
| **G5** | A revoked pass is refused at ENTRY in every configuration of terms, and revocation is terminal: no transition leaves revoked, and revoking a pass ends its registrations on the revocation day in the local calendar of EACH GARAGE THE PASS NAMES -- the rows at each garage on that garage's day of the instant, never on the day at the garage the command was typed from -- so the identity is free from that day there; every garage's day is derived before the first row changes, so a garage of the pass whose stored zone cannot be read refuses the revocation by name and nothing is half-ended. |
...
| **G9** | A visit allowance and a maximum stay are computed from the module's OWN recorded visits and from nothing else, and the answer names its denominator: how many entries were counted, on which pass, over its life or in which window on which day -- EACH ENTRY READ IN THE LOCAL DAY OF THE GARAGE IT WAS RECORDED AT, the count naming how many at each, because a pass's garages can stand in different zones and the wall clock an entry fell in is the one on the door it drove through (the LIFE allowance reads no clock); and which entry a stay was measured from -- the one recorded AT THE GARAGE THE CAR IS LEAVING, never an entry open at another garage of the pass -- both instants rendered as the garage's wall clock whatever offset each arrived with. A maximum stay with no open recorded entry AT THIS GARAGE to measure from is UNMEASURED and the answer says so by name while answering on the terms that can be evaluated -- never silently treated as satisfied, and never a refusal. |
...
| **G25** | A PASS ANSWERS ONLY AT THE GARAGES IT NAMES. A pass carries a non-empty SET of garages, stated by listing them -- never an 'everywhere' flag, never inferred, an empty set or a blank member refused at creation naming the field -- and every door reads that set by MEMBERSHIP: the access call selects a pass for a garage only if the pass names it (at both places a pass is read for its garage, the duplicated-id grouping and the by-id map), the store refuses to store a pass at a garage it does not name and to load it at one, and a QR or a holder link of the pass redeems at any garage of the set and at no other, refused by name elsewhere. The terms are ONE set on the pass, evaluated at whichever garage the car is at -- a 20-visit allowance is 20 on the pass, not 20 per garage -- except the lanes, which are stated PER GARAGE because lane A1 at two garages is two barriers: where lanes are stated every garage the pass names has its own set, refused at creation otherwise naming the garage, and the lane check at each garage reads its own set and no other. A pass's id is unique per tenant, not per garage. Migration 0004 BACKFILLS: every pass that existed keeps exactly the one garage it had and every lane row carries its own pass's garage; a lane row it cannot place makes it fail naming the count rather than drop the row, and two passes of one tenant sharing an id make it fail naming the id rather than pick a winner. THE VISIT LEDGER STAYS PER GARAGE: one open visit per vehicle per pass PER GARAGE, by the module's own check and by the database's index, so an entry at one garage of the set is never refused for a visit still open at another and an exit never closes another garage's visit -- while the allowance still counts every garage of the set, EACH ENTRY READ ON THE CLOCK OF THE GARAGE IT WAS RECORDED AT: the pass's garages reach the engine (the store hands in every garage of every pass it selected; the command line hands in --garages), a per-window count reads each entry's day and minute in its own garage's zone against the asking garage's day, and an entry at a garage whose clock is not there -- not handed in, handed in unreadable, handed in twice -- is refused by name at an entry, never read on the asking garage's clock and never dropped (no exit counts an allowance, so no exit reaches it) -- and that refusal names the entry it could not read by its ABSOLUTE instant, as UTC and said so, unrendered on that garage's wall clock because that is the clock it does not hold: never in the offset the value arrived with, which through the store is the database session's zone, the same bytes under any session zone. A visit's garage id is text through the module's one validator, as a garage's id and each member of the pass's set are -- stripped, so an entry recorded under an id with a scanner's trailing space matches the garage handed in under the stripped id and is counted; blank or carrying a control character it is refused by name at construction, naming visit.garage_id. And a membership row decides nothing recorded under it: removing a garage from a pass that holds a visit or a registration there fails by name (the keys are RESTRICT), never erases them; only the lanes stated at that garage go with it. |
| **G26** | A PASS CAN BE READ WHOLE, BY ANY READER, AND THE READ WRITES NOTHING. One verb, show-pass, with the arguments every store verb takes (--tenant, --garage, --pass-id), prints as JSON the pass id, its stored state, the garages it names (external ids, sorted) and EVERY registration of the pass, history included -- ended rows too -- each as the identity as stored, the garage's external id, effective_day, end_day and ended_reason -- and the two days the terms bound the pass with, valid_from and valid_to, as ISO days or null; the library function under the verb is the same code the verb calls, never a second path. It answers that and nothing more: not the holder's name, phone or email, no term beyond the two valid days (not the windows, the maximum stay, the allowance, the lanes or the directions), not the label, nothing from enrolments or holder_links -- the reader needs the register and needs to know WHEN the pass covers, not HOW, and personal data it does not need does not travel. The read uses no clock: it derives no expired and takes no as-of day -- a pass spans garages in different timezones and which day is the reader's call -- so the reader compares the two days with its own; a null bound is a bound the terms did not state, or terms the module cannot read, which unreadable already names. The rows are selected by the pass alone, never through its garage set, so no row is dropped for its garage; and the database says such a row cannot exist: a registration names a garage the pass holds (vehicle_registrations_garage_of_pass, migration 0004, RESTRICT), so a raw insert at a garage outside the set is refused by the constraint and a membership row under a registration cannot be removed. The order is applied IN PYTHON, by code point -- identity, then effective day, then garage external id; the garage list the same way -- never by ORDER BY, because the database's collation orders the same text differently on macOS and on glibc and a reader on another machine must get the same bytes. A pass stored unreadable (G17) is still shown -- state, garages and registrations -- and the unreadable field is named with its code; a garage of the set stored with a timezone the running system does not carry stops no read, since the read uses no clock: the pass is shown and each such garage is named with its code. A pass that does not exist, or exists only at another tenant, is refused by name as every store verb refuses it; a garage the pass does not name is refused by name naming the set (G25 -- the read is not a way to look a pass up from a garage it does not answer at); a garage the tenant does not have is refused by name. The read writes nothing, proven by row counts and row digests of every table in the catalogue before and after, never by reading the code's order. Another tenant's pass with the same id is not shown, proven beside a known-shown control in the same run, and held twice: by the row policy and by the read's own tenant predicate, each proven to hold with the other removed. The token is not rendered by the read and neither is its digest (G23's rendered scan covers the verb, with a plant as its positive control). The module stays standalone: nothing in the verb, its output, the contract, the README or any refusal names any other module or any particular reader -- it is a read of a pass for any reader. |

That is 26 guarantees. Every one of them has a fail control that has been proven to fire, and the count above is derived from the registry rather than typed here.
<!-- END:guarantees -->
...
| `garages` | A per-window visit allowance counts each recorded entry in the local day of THE GARAGE IT WAS RECORDED AT -- a pass names a set of garages, and two of them can stand in different zones -- and an entry on this pass was recorded at a garage that is not among the garages handed in, so its day and window cannot be read. The module will not read it on this garage's clock instead: measured before this, a visit at a Tokyo garage read on a Denver clock fell on the day before and a one-per-window allowance was spent twice. Hand in every garage the pass names. (Only an ENTRY counts the allowance, so no exit reaches this.) |
| `garages[].id` | The garage an entry on this pass was recorded at was handed in more than once under one id, at an ENTRY that must read that entry on that garage's clock -- whether or not the copies agree, since the caller who sent one id twice does not know which they meant. Hand in each garage once. (Only an ENTRY counts the allowance, so no exit reaches this.) |
...
| visit | `entered_at`, `entry_lane`, `exit_lane`, `exited_at`, `garage_id`, `pass_id`, `vehicle_identity` |
...
- `CAR-1` at lane `L1`, **entry**, `2026-06-01T12:00:00-06:00` → **covered** by `pass-employee-7` (Employee): valid 2026-01-01..2026-12-31; windows Mon,Tue,Wed,Thu,Fri 06:00-20:00; max stay 10:00:00; 3 visit(s) per window; directions entry+exit; lanes garage-downtown:L1,L2; in window Mon,Tue,Wed,Thu,Fri 06:00-20:00 on 2026-06-01; visit 3 of 3; counted 2 recorded entries on pass 'pass-employee-7' in window Mon,Tue,Wed,Thu,Fri 06:00-20:00 on 2026-06-01, each read in the local day of the garage it was recorded at (garage-downtown: 2)
