...
GUARANTEES: dict[str, str] = {
    ...
    "G26": (
        ...
        "ended_reason -- and the two days the terms bound the pass with, valid_from and "
        "valid_to, as ISO days or null; the library function under the verb is the same code "
        "the verb calls, never a second path. It answers that and nothing more: not the "
        "holder's name, phone or email, no term beyond the two valid days (not the windows, "
        "the maximum stay, the allowance, the lanes or the directions), not the label, "
        "nothing from enrolments or holder_links -- the reader needs the register and needs "
        "to know WHEN the pass covers, not HOW, and personal data it does not need does not "
        "travel. The read uses no clock: it derives no expired and takes no as-of day -- a "
        "pass spans garages in different timezones and which day is the reader's call -- so "
        "the reader compares the two days with its own; a null bound is a bound the terms did "
        "not state, or terms the module cannot read, which unreadable already names. "
