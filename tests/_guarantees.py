...
GUARANTEES: dict[str, str] = {
    ...
    "G19": (
        ...
        "redeemed and the access answer -- all or none, under a savepoint, so a refusal "
        "anywhere, the module's own or the database's backstop, writes nothing and a QR "
        "cannot half-land and be used twice. days_valid is STATED, refused by name when "
        "absent, never defaulted. The holder link is the same primitive, used once. "
        "Revoking a pass cancels every outstanding enrolment and holder link on it in the "
        "same transaction, and the redemption's own state check refuses a revoked pass "
        "even with that cancellation planted away."
    ),
    "G20": (
    ...
    "G21": (
        ...
        "a pass that is not registrable, a lane outside the pass's terms, an identity on "
        "another pass, an unreadable garage. At an exit lane that answer is covered or "
        "not-covered with the exit note, never refused-to-answer (G4 holds through the "
        "enrolment path); at an entry it is whatever access says. The census of every "
        "refusal kind at every lane end is reported N/N with zero unanswered exits."
    ),
    "G22": (
    ...
    "G24": (
        ...
        "delivering is the integrator's."
    ),
}
