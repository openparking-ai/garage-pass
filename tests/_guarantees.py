...
GUARANTEES: dict[str, str] = {
    "G1": (
        ...
        "refused BY NAME, naming the pass that holds the identity and the day that "
        "registration ends, BEFORE the database constraint has to -- and a refusal "
        "writes nothing. The database's EXCLUDE is the backstop for a raw insert and "
        "for two registrations racing. Ending a registration on day D frees the "
        "identity FROM D: a registration elsewhere effective D is accepted, and the "
        "old pass covers the vehicle up to and not including D."
    ),
    "G2": (
