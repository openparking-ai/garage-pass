...
GUARANTEES: dict[str, str] = {
    ...
    "G4": (
        ...
        "pass that was not handed in. Every exit answer carries the sentence that says "
        "so, a not-covered exit means OUT-OF-TERMS and never a refusal, in every state "
        "including revoked, and the exit half of the call does not read the garage's "
        "transient mode. What remains outside this sentence is named as such and is "
        "not data: two CALLER-CONTRACT errors -- an instant with no timezone and a "
        "direction that is not a Direction -- which raise on the first call with no "
        "instant to answer about, and one ENVIRONMENT error -- a machine with no "
        "timezone database at all -- which is raised by its own name, never as an "
        "unknown zone. A garage carrying a timezone this system does not carry cannot "
        "be constructed, so an exit at such a garage is not a path that exists; a "
        "STORED one loads unreadable and answers."
    ),
    "G5": (
    ...
    "G18": (
        ...
        "with exit 2 -- as is a machine with no timezone database at all, named as "
        "that and never as an unknown zone. A value that starts with a dash "
        "(`--timezone -06:00`) reaches the module and is refused by name, not read by "
        "argparse as an option. Every raise in the package is enumerated by AST and "
        "classified as rendered at that boundary, as the machine's configuration, or as "
        "a programming error no command can reach -- the two caller-contract errors, a "
        "naive instant and a wrong-typed direction, are reachable only from the pure "
        "API and never from a command, and are dead by decision; an exception class in "
        "none of the three fails the suite until it is classified."
    ),
    "G16": (
